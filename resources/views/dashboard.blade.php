...
<flux:main class="min-h-screen space-y-6 bg-[#FFFDF8] p-4 font-['Inter'] md:p-6">
    ...
    @php
        ...
        // ── KPI cards (8) ──
        $kpis = [
            ['label' => 'Total Employees', 'value' => $totalActive, 'icon' => 'users', 'accent' => 'orange', 'delta' => 3, 'dir' => 'up', 'compare' => 'vs last month', 'spark' => $spark($totalActive)],
            ['label' => 'Present Today', 'value' => $presentToday, 'icon' => 'check-circle', 'accent' => 'green', 'delta' => $presentPct, 'dir' => 'up', 'compare' => 'of '.$totalActive.' active', 'spark' => $spark(max($presentToday, 1))],
            ...
            ['label' => 'Open Approvals', 'value' => $totalPending, 'icon' => 'inbox-stack', 'accent' => 'red', 'delta' => 5, 'dir' => 'up', 'compare' => $pendingLeavesCount.' leave · '.$pendingOtCount.' OT', 'spark' => $spark(max($totalPending, 1))],
            ...
            ['label' => 'Compliance', 'value' => $complianceScore.'%', 'icon' => 'shield-check', 'accent' => 'green', 'delta' => 2, 'dir' => 'up', 'compare' => 'docs + reviews', 'spark' => $spark($complianceScore, 0.08)],
        ];
        ...
        <div class="relative flex flex-col items-start justify-between gap-6 lg:flex-row lg:items-center">
            <div>
                <div class="mb-2 inline-flex items-center gap-2 rounded-full border border-[#F3E8DD] bg-white px-3 py-1 text-[11px] font-semibold text-[#6B7280]">
                    <flux:icon.sun class="size-3.5 text-orange-500" /> {{ now()->format('l, d M Y') }} · 28°C Sunny · {{ $scope }} Overview
                </div>
                <h1 class="text-[28px] font-extrabold tracking-tight text-[#111827]">{{ $greeting }}, {{ $firstName }} 👋</h1>
                <p class="mt-1 text-sm text-[#6B7280]">Everything looks healthy today — {{ $totalPending }} approvals need your attention.</p>
            </div>
    ...
    <div class="grid grid-cols-1 gap-6 lg:grid-cols-12">
        <div class="dash-rise rounded-2xl border border-[#F3E8DD] bg-white p-6 shadow-sm transition hover:shadow-lg lg:col-span-8">
            <div class="mb-4 flex items-center justify-between">
                <div>
                    <h3 class="text-lg font-bold tracking-tight text-[#111827]">Attendance Analytics</h3>
                    <p class="text-sm text-[#6B7280]">Monthly attendance rate · last 6 months</p>
                </div>
        ...
        <div class="dash-rise flex flex-col rounded-2xl border border-[#F3E8DD] bg-white p-6 shadow-sm transition hover:shadow-lg lg:col-span-4">
            <h3 class="text-lg font-bold tracking-tight text-[#111827]">Compliance Score</h3>
            <p class="text-sm text-[#6B7280]">HR health &amp; risk meter</p>
            <x-dashboard.chart :options="$gauge" class="-mt-1" />
            <div class="mt-auto grid grid-cols-2 gap-3">
                <div class="rounded-xl bg-green-50 p-3 text-center"><div class="text-base font-extrabold text-green-600">{{ $complianceScore >= 80 ? 'Low' : ($complianceScore >= 60 ? 'Medium' : 'High') }}</div><div class="text-[11px] font-medium text-[#6B7280]">Risk level</div></div>
                <div class="rounded-xl bg-orange-50 p-3 text-center"><div class="text-base font-extrabold text-orange-600">{{ $complianceScore >= 80 ? 'Excellent' : 'Watch' }}</div><div class="text-[11px] font-medium text-[#6B7280]">HR health</div></div>
            </div>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 gap-6 lg:grid-cols-12">
        ...
        <div class="dash-rise space-y-6 lg:col-span-4">
            ...
            <div class="rounded-2xl border border-[#F3E8DD] bg-white p-6 shadow-sm">
                <div class="mb-3 flex items-center justify-between">
                    <h3 class="text-base font-bold tracking-tight text-[#111827]">Pending Approvals</h3>
                ...
                <div class="space-y-2">
                    @foreach($pendingApprovals as $pa)
                        <a href="{{ $pa['href'] }}" wire:navigate class="flex items-center justify-between rounded-xl border border-[#F3E8DD] bg-[#FFFDF8] px-3 py-2.5 transition hover:border-orange-200 hover:bg-[#FFF2E8]/60">
                            <span class="text-sm font-medium text-[#374151]">{{ $pa['label'] }}</span>
                            <span class="text-sm font-extrabold {{ $pa['count'] > 0 ? 'text-orange-500' : 'text-[#D8CCBE]' }}">{{ $pa['count'] }}</span>
                        </a>
                    @endforeach
                </div>
            </div>
            ...
            <div class="overflow-hidden rounded-2xl border border-orange-200/70 bg-gradient-to-br from-[#FFF8F1] to-white p-6 shadow-sm">
                <div class="mb-3 flex items-center gap-2">
                    ...
                    <h3 class="text-base font-bold tracking-tight text-[#111827]">AI Insights</h3>
                ...
                <div class="space-y-2.5">
                    @foreach(array_slice($insights, 0, 4) as $in)
                        <div class="flex items-start gap-3 rounded-xl border border-[#F3E8DD] bg-white p-3">
                            ...
                            <div class="min-w-0"><div class="text-[13px] font-bold text-[#111827]">{{ $in['title'] }}</div><p class="text-[11px] leading-relaxed text-[#6B7280]">{{ $in['text'] }}</p></div>
                        </div>
                    @endforeach
                </div>
            </div>
            ...
            {{-- Upcoming Events --}}
            <div class="rounded-2xl border border-[#F3E8DD] bg-white p-6 shadow-sm">
                <h3 class="mb-4 text-base font-bold tracking-tight text-[#111827]">Upcoming Events</h3>
                <div class="space-y-3">
                    @forelse($upcomingBirthdays->take(4) as $b)
                        <div class="flex items-center gap-3 rounded-xl p-2 transition hover:bg-[#FFF2E8]/60">
                            ...
                            <div class="min-w-0 flex-1"><div class="truncate text-sm font-semibold text-[#111827]">{{ $b['name'] }}</div><div class="truncate text-[11px] text-[#6B7280]">🎂 {{ $b['dept'] ?: 'Birthday' }}</div></div>
                            <span class="shrink-0 rounded-lg bg-[#FFFDF8] px-2 py-1 text-[11px] font-bold text-orange-500">{{ $b['is_today'] ? 'Today' : 'in '.$b['days'].'d' }}</span>
                        </div>
                    @empty
                        <p class="py-4 text-center text-sm text-[#9CA3AF]">No birthdays in the next 30 days.</p>
                    @endforelse
                    <div class="flex items-center gap-3 rounded-xl p-2">
                        ...
                        <div class="min-w-0 flex-1"><div class="text-sm font-semibold text-[#111827]">Payroll run</div><div class="text-[11px] text-[#6B7280]">Month-end cycle</div></div>
                        <span class="shrink-0 rounded-lg bg-[#FFFDF8] px-2 py-1 text-[11px] font-bold text-red-500">{{ now()->endOfMonth()->format('d M') }}</span>
                    </div>
                </div>
            </div>
        </div>
    ...
    <div class="grid grid-cols-1 gap-6 lg:grid-cols-12">
        <div class="dash-rise rounded-2xl border border-[#F3E8DD] bg-white p-6 shadow-sm transition hover:shadow-lg lg:col-span-4">
            <h3 class="mb-1 text-lg font-bold tracking-tight text-[#111827]">Department Distribution</h3>
            <p class="text-sm text-[#6B7280]">Active employees by team</p>
            @if($deptDist->isNotEmpty())<x-dashboard.chart :options="$donut" />@else<p class="py-16 text-center text-sm text-[#9CA3AF]">No department data.</p>@endif
        </div>

        <div class="dash-rise overflow-hidden rounded-2xl border border-[#F3E8DD] bg-white p-6 shadow-sm lg:col-span-8">
            <div class="mb-5 flex flex-wrap items-center justify-between gap-3">
                <h3 class="text-lg font-bold tracking-tight text-[#111827]">Team Attendance Heatmap</h3>
                <div class="flex flex-wrap items-center gap-3 text-[11px] font-semibold text-[#6B7280]">
                    @foreach(['Present' => 'bg-green-500', 'Late' => 'bg-amber-500', 'Absent' => 'bg-red-400', 'Off' => 'bg-[#EFE4D7]'] as $l => $c)
                ...
                <table class="w-full">
                    <thead>
                        <tr class="text-[11px] font-bold uppercase tracking-wide text-[#9CA3AF]">
                            ...
                            @foreach($days as $d)
                                <th class="px-2 pb-3 text-center {{ $d->isWeekend() ? 'text-[#D8CCBE]' : '' }}"><div>{{ $d->format('D') }}</div><div class="text-[10px] font-medium {{ $d->isToday() ? 'text-orange-500' : '' }}">{{ $d->format('j') }}</div></th>
                            @endforeach
                        </tr>
                    </thead>
                    <tbody>
                            ...
                            <tr class="group">
                                <td class="py-1.5 pr-4">
                                    <div class="flex items-center gap-2.5">
                                        <span class="flex size-7 items-center justify-center rounded-full bg-[#FFFDF8] text-[10px] font-bold text-orange-500 ring-1 ring-[#F3E8DD]">{{ $row['initials'] }}</span>
                                        <span class="truncate text-xs font-semibold text-[#111827]">{{ $row['name'] }}</span>
                                    </div>
                                </td>
    ...
    <div class="grid grid-cols-1 gap-6 lg:grid-cols-12">
        <div class="dash-rise rounded-2xl border border-[#F3E8DD] bg-white p-6 shadow-sm lg:col-span-7">
            <div class="mb-4 flex items-center justify-between">
                <h3 class="text-lg font-bold tracking-tight text-[#111827]">Recent Activity</h3>
        ...
        <div class="dash-rise space-y-6 lg:col-span-5">
            <div class="rounded-2xl border border-[#F3E8DD] bg-white p-6 shadow-sm">
                <h3 class="mb-4 text-lg font-bold tracking-tight text-[#111827]">Compliance Alerts</h3>
                <div class="grid grid-cols-2 gap-3">
                    @foreach($complianceAlerts as $al)
                        @php $t = $tone($al['tone']); @endphp
                        <a href="{{ $al['href'] }}" wire:navigate class="rounded-xl border border-[#F3E8DD] bg-[#FFFDF8] p-4 transition hover:-translate-y-0.5 hover:shadow-md">
                            <div class="flex items-center justify-between">
                                <span class="rounded-lg {{ $toneSoft[$t] }} px-2 py-0.5 text-[10px] font-bold uppercase">{{ $al['status'] }}</span>
                                <span class="text-2xl font-extrabold {{ $al['count'] > 0 ? $toneText[$t] : 'text-[#D8CCBE]' }}">{{ $al['count'] }}</span>
                            </div>
                            <div class="mt-2 text-xs font-semibold leading-tight text-[#6B7280]">{{ $al['label'] }}</div>
                        </a>
                    @endforeach
                </div>
            </div>
            ...
            {{-- Quick Actions --}}
            <div class="rounded-2xl border border-[#F3E8DD] bg-white p-6 shadow-sm">
                <h3 class="mb-4 text-lg font-bold tracking-tight text-[#111827]">Quick Actions</h3>
                <div class="grid grid-cols-2 gap-3">
                    @foreach([
