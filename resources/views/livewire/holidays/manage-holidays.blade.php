...
<div class="mb-4 flex flex-wrap items-center justify-between gap-3">
    <div>
        <h1 class="text-2xl font-black tracking-tight text-zinc-900">Holiday Management</h1>
...
<div class="mb-4 grid grid-cols-2 gap-3 sm:grid-cols-4">
    @foreach([['Total Holidays', $stats['total'], 'calendar-days', '#F97316'], ['Paid', $stats['paid'], 'banknotes', '#10b981'], ['Optional', $stats['optional'], 'hand-raised', '#0ea5e9'], ['Upcoming', $stats['upcoming'], 'clock', '#8b5cf6']] as [$label, $value, $icon, $color])
        <div class="flex items-center gap-3 rounded-[18px] border border-orange-100/70 bg-white p-4 shadow-sm">
            ...
            <div><div class="text-xl font-black tabular-nums text-zinc-900">{{ $value }}</div><div class="text-[9px] font-bold uppercase tracking-wide text-zinc-400">{{ $label }}</div></div>
        </div>
    @endforeach
</div>

<div class="mb-4 flex flex-wrap items-center gap-2 rounded-[18px] border border-orange-100/70 bg-white p-3 shadow-sm">
    {{-- View toggle --}}
    <div class="flex items-center gap-1 rounded-xl bg-orange-50/60 p-1">
        @foreach(['calendar' => 'Month', 'list' => 'List', 'year' => 'Year'] as $v => $vl)
            <button wire:click="$set('view', '{{ $v }}')" class="rounded-lg px-3 py-1.5 text-xs font-bold transition {{ $view === $v ? 'bg-orange-500 text-white shadow' : 'text-zinc-500 hover:text-orange-500' }}">{{ $vl }}</button>
        @endforeach
    </div>
...
<div class="mb-3 flex flex-wrap items-center gap-3 text-[10px] font-bold">
    @foreach($types as $t)
        <span class="inline-flex items-center gap-1.5 text-zinc-500"><span class="size-2.5 rounded-full" style="background: {{ $t['color'] }}"></span>{{ $t['label'] }}</span>
    @endforeach
</div>
...
@if($view === 'calendar')
    <div class="rounded-[18px] border border-orange-100/70 bg-white p-5 shadow-sm">
        <div class="mb-3 flex items-center justify-between">
            <div class="text-sm font-black text-zinc-900">{{ $monthLabel }}</div>
        ...
        <div class="mt-1 grid grid-cols-7 gap-1">
            @foreach($calendarDays as $day)
                <div wire:click="openCreate('{{ $day['date'] }}')"
                    class="group min-h-[76px] cursor-pointer rounded-xl border p-1.5 transition hover:border-orange-300 hover:shadow-sm {{ $day['inMonth'] ? 'border-zinc-100 bg-white' : 'border-transparent bg-zinc-50/40' }} {{ $day['isToday'] ? 'ring-2 ring-orange-300' : '' }}">
                    <div class="flex items-center justify-between">
                        <span class="text-xs font-bold {{ $day['inMonth'] ? ($day['isWeekend'] ? 'text-zinc-400' : 'text-zinc-700') : 'text-zinc-300' }}">{{ $day['day'] }}</span>
                        ...
                        <tr class="transition hover:bg-orange-50/40">
                            <td class="px-5 py-2.5">
                                <button wire:click="showDetail({{ $h->id }})" class="flex items-center gap-2 text-left">
                                    ...
                                    <span class="font-black text-zinc-900 {{ $h->is_active ? '' : 'line-through opacity-50' }}">{{ $h->name }}</span>
                                </button>
                            </td>
                            <td class="py-2.5"><div class="font-bold text-zinc-700">{{ $h->date->format('d M Y') }}</div><div class="text-[9px] text-zinc-400">{{ $h->date->format('l') }}</div></td>
                            ...
                            <td class="py-2.5 text-zinc-500">{{ $h->office?->name ?? ($h->department?->name ?? 'Company-wide') }}</td>
                            <td class="py-2.5">
                                <div class="flex flex-wrap gap-1">
                                    ...
                                    @unless($h->is_active)<span class="rounded bg-zinc-100 px-1.5 py-0.5 text-[8px] font-bold text-zinc-500">Archived</span>@endunless
                                </div>
                            </td>
                            <td class="px-5 py-2.5">
        ...
        @for($m = 1; $m <= 12; $m++)
            ...
            <div class="rounded-[18px] border border-orange-100/70 bg-white p-4 shadow-sm">
                <div class="mb-2 flex items-center justify-between">
                    <div class="text-sm font-black text-zinc-900">{{ \Carbon\Carbon::create($year, $m, 1)->format('F') }}</div>
                ...
                @forelse($mh->sortBy('date') as $h)
                    <button wire:click="showDetail({{ $h->id }})" class="mb-1 flex w-full items-center gap-2 rounded-lg px-2 py-1 text-left text-xs transition hover:bg-orange-50/60">
                        ...
                        <span class="truncate font-semibold text-zinc-700">{{ $h->name }}</span>
                    </button>
                @empty
    ...
    <div class="fixed inset-0 z-50 flex items-center justify-center p-4" x-data x-on:keydown.escape.window="$wire.set('showForm', false)">
        ...
        <div class="relative max-h-[90vh] w-full max-w-lg overflow-y-auto rounded-2xl bg-white p-6 shadow-xl ring ring-black/5">
            <div class="mb-4 flex items-center justify-between">
                ...
                <button @click="$wire.set('showForm', false)" class="text-zinc-400 hover:text-zinc-600"><flux:icon.x-mark class="size-5" /></button>
            </div>
            <div class="space-y-3">
                ...
                <div class="flex flex-wrap gap-4 rounded-xl bg-orange-50/40 px-3 py-2.5">
                    <label class="flex items-center gap-2 text-xs font-semibold text-zinc-600"><input type="checkbox" wire:model="form.is_paid" class="rounded border-zinc-300 text-orange-500 focus:ring-orange-400"> Paid Holiday</label>
                    <label class="flex items-center gap-2 text-xs font-semibold text-zinc-600"><input type="checkbox" wire:model="form.is_optional" class="rounded border-zinc-300 text-orange-500 focus:ring-orange-400"> Optional</label>
                    <label class="flex items-center gap-2 text-xs font-semibold text-zinc-600"><input type="checkbox" wire:model="form.is_recurring" class="rounded border-zinc-300 text-orange-500 focus:ring-orange-400"> Recurring Yearly</label>
                </div>
            ...
            <div class="mt-5 flex justify-end gap-2">
                <button @click="$wire.set('showForm', false)" class="rounded-xl border border-zinc-200 px-4 py-2 text-sm font-semibold text-zinc-600 transition hover:bg-zinc-50">Cancel</button>
...
@if($detail)
    <div class="fixed inset-0 z-50 flex items-center justify-center p-4" x-data x-on:keydown.escape.window="$wire.set('detail', null)">
        ...
        <div class="relative w-full max-w-sm overflow-hidden rounded-2xl bg-white shadow-xl ring ring-black/5">
            ...
            <div class="space-y-2 p-5 text-xs">
                @if($detail['description'])<p class="text-zinc-600">{{ $detail['description'] }}</p>@endif
                <div class="flex items-center justify-between"><span class="text-zinc-400">Scope</span><span class="font-bold text-zinc-800">{{ $detail['scope'] }}</span></div>
                <div class="flex items-center justify-between"><span class="text-zinc-400">Country</span><span class="font-bold text-zinc-800">{{ $detail['country'] }}</span></div>
                @if($detail['category'])<div class="flex items-center justify-between"><span class="text-zinc-400">Category</span><span class="font-bold text-zinc-800">{{ $detail['category'] }}</span></div>@endif
                <div class="flex flex-wrap gap-1.5 pt-1">
                    ...
                    @unless($detail['is_active'])<span class="rounded-full bg-zinc-100 px-2 py-0.5 text-[9px] font-bold text-zinc-500">Archived</span>@endunless
                </div>
                @if($detail['creator'])<div class="border-t border-zinc-100 pt-2 text-[10px] text-zinc-400">Added by {{ $detail['creator'] }}</div>@endif
            </div>
            <div class="flex justify-end gap-2 border-t border-zinc-100 px-5 py-3">
                <button @click="$wire.set('detail', null)" class="rounded-xl border border-zinc-200 px-4 py-2 text-sm font-semibold text-zinc-600 transition hover:bg-zinc-50">Close</button>
