<flux:main class="bg-zinc-50 dark:bg-zinc-800/50 p-6 space-y-6 dark:bg-zinc-950">
        ...
        <div class="flex items-center gap-2">
            @if(auth()->user()->employee)
                <button type="button" onclick="document.getElementById('modal-personal').showModal()"
                    class="inline-flex items-center gap-2 rounded-lg border border-zinc-300 bg-white dark:bg-zinc-900 px-4 py-2 text-sm font-medium text-zinc-700 dark:text-zinc-200 hover:bg-zinc-50 dark:bg-zinc-800/50 shadow-sm transition-colors">
    ...
    @endphp
    <div class="flex w-fit items-center gap-1 rounded-xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 p-1 shadow-sm dark:border-zinc-800 dark:bg-zinc-900">
        @foreach($docTabs as $val => $label)
            <button wire:click="setView('{{ $val }}')"
                class="rounded-lg px-4 py-1.5 text-sm font-bold transition-all
                    {{ $view === $val ? 'bg-zinc-900 text-white dark:bg-white dark:text-zinc-900 dark:text-white' : 'text-zinc-500 dark:text-zinc-400 hover:text-zinc-700 dark:text-zinc-200 dark:hover:text-zinc-300' }}">
                {{ $label }}
            </button>
        @endforeach
    </div>
    ...
    <div class="flex flex-wrap items-center gap-3">
        <div class="relative">
            ...
            <input wire:model.live.debounce.300ms="filterSearch" type="text" placeholder="Search documents…"
                class="pl-9 pr-4 h-9 rounded-lg border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 text-sm focus:outline-none focus:ring-2 focus:ring-orange-400 w-64 dark:border-zinc-700 dark:bg-zinc-800 dark:text-zinc-100" />
        </div>
        ...
        @if($canManage)
            ...
            <div class="flex items-center gap-1.5">
                ...
                <input type="date" wire:model.live="filterExpiringFrom" aria-label="Expiring from"
                    class="h-9 rounded-lg border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 px-2 text-sm focus:outline-none focus:ring-2 focus:ring-orange-400 dark:border-zinc-700 dark:bg-zinc-800 dark:text-zinc-100" />
    ...
    <div class="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
        @forelse($documents as $doc)
            <div class="flex flex-col justify-between rounded-xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 p-5 shadow-sm hover:shadow-md transition-shadow dark:border-zinc-800 dark:bg-zinc-900">
                <div>
                    {{-- Title + badges --}}
                    <div class="flex items-start justify-between gap-2 mb-3">
                        <h3 class="font-semibold text-zinc-900 dark:text-white text-sm leading-snug line-clamp-2">{{ $doc->title }}</h3>
                        <div class="flex shrink-0 flex-col items-end gap-1">
                            <span class="rounded-md bg-zinc-100 dark:bg-zinc-800 px-2 py-0.5 text-[10px] font-bold uppercase text-zinc-500 dark:text-zinc-400 dark:bg-zinc-800 dark:text-zinc-400">v{{ $doc->version }}</span>
                    ...
                    @if($doc->description)
                        <p class="text-xs text-zinc-500 dark:text-zinc-400 mb-3 line-clamp-2">{{ $doc->description }}</p>
                    @endif
                    ...
                    <div class="space-y-1 text-xs text-zinc-400 mb-4">
                        ...
                        @if($canManage && $doc->employee)
                            <div class="text-zinc-500 dark:text-zinc-400">For: {{ $doc->employee->user->name }}</div>
                        @endif
                    </div>
                </div>

                {{-- Actions --}}
                <div class="flex items-center justify-between border-t border-zinc-100 dark:border-zinc-800 pt-3 dark:border-zinc-800">
                    {{-- View + Download --}}
                    <div class="flex flex-wrap items-center gap-3">
                        <a href="{{ URL::temporarySignedRoute('documents.view', now()->addMinutes(5), ['document' => $doc->id]) }}"
                            target="_blank"
                            class="inline-flex items-center gap-1.5 text-sm text-zinc-600 dark:text-zinc-300 hover:text-orange-600 font-medium transition-colors">
                        ...
                        @if($canManage)
                            <div x-data="{ open: false }" class="relative">
                                <button @click="open = !open" type="button"
                                    class="rounded-lg p-1.5 text-zinc-400 hover:bg-zinc-100 dark:bg-zinc-800 hover:text-zinc-600 dark:text-zinc-300 transition-colors dark:hover:bg-zinc-800">
                                ...
                                <div x-show="open" @click.outside="open = false" x-transition
                                    class="absolute right-0 bottom-8 z-50 w-44 rounded-xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 shadow-lg py-1 dark:border-zinc-700 dark:bg-zinc-900">
                                    <button type="button"
                                        @click="open = false; document.getElementById('new-version-id').value = '{{ $doc->id }}'; document.getElementById('modal-upload').showModal()"
                                        class="w-full flex items-center gap-2 px-4 py-2 text-sm text-zinc-700 dark:text-zinc-200 hover:bg-zinc-50 dark:bg-zinc-800/50 dark:text-zinc-300 dark:hover:bg-zinc-800">
        ...
        @empty
            <div class="col-span-full rounded-xl border border-dashed border-zinc-200 dark:border-zinc-800 py-16 text-center dark:border-zinc-800">
                <svg class="mx-auto mb-3 size-12 text-zinc-200 dark:text-zinc-700 dark:text-zinc-200" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/></svg>
                ...
                <div class="pulse-card">
                    <div class="mb-4 flex items-center justify-between">
                        ...
                        <span class="rounded-full px-2 py-0.5 text-xs font-black
                            {{ $key === 'expired' ? 'bg-rose-100 text-rose-600 dark:bg-rose-950/40 dark:text-rose-400' : 'bg-zinc-100 dark:bg-zinc-800 text-zinc-500 dark:text-zinc-400 dark:bg-zinc-800 dark:text-zinc-400' }}">
                            {{ $expiry[$key]->count() }}
                        </span>
                    </div>
                    <div class="space-y-3">
                    ...
                    @if($row['pending'] > 0)
                        <div class="mt-3 flex flex-wrap gap-1.5">
                            @foreach($row['pendingNames'] as $name)
                                <span class="rounded-full bg-zinc-100 dark:bg-zinc-800 px-2 py-0.5 text-[11px] font-medium text-zinc-500 dark:text-zinc-400 dark:bg-zinc-800 dark:text-zinc-400">{{ $name }}</span>
                            @endforeach
    ...
    @if($versionList !== null)
        <div class="fixed inset-0 z-50 flex items-center justify-center p-4" x-data x-on:keydown.escape.window="$wire.closeVersions()">
            ...
            <div class="relative w-full max-w-lg rounded-2xl bg-white dark:bg-zinc-900 shadow-xl ring-1 ring-black/5 dark:bg-zinc-900 dark:ring-zinc-700">
                <div class="flex items-center justify-between border-b border-zinc-100 dark:border-zinc-800 px-5 py-4 dark:border-zinc-800">
                    <h3 class="text-sm font-bold text-zinc-900 dark:text-white">Version History</h3>
                    <button type="button" @click="$wire.closeVersions()" class="text-zinc-400 hover:text-zinc-600 dark:text-zinc-300 dark:hover:text-zinc-200">
                ...
                <div class="max-h-[70vh] divide-y divide-zinc-50 overflow-y-auto dark:divide-zinc-800/60">
                    @foreach($versionList as $v)
                        <div class="flex items-center justify-between gap-3 px-5 py-3">
                            <div class="min-w-0">
                                <div class="flex items-center gap-2">
                                    <span class="rounded-md bg-zinc-100 dark:bg-zinc-800 px-2 py-0.5 text-[10px] font-bold uppercase text-zinc-500 dark:text-zinc-400 dark:bg-zinc-800 dark:text-zinc-400">v{{ $v->version }}</span>
                                    @if($loop->first)
                            ...
                            <div class="flex shrink-0 items-center gap-3">
                                <a href="{{ URL::temporarySignedRoute('documents.view', now()->addMinutes(5), ['document' => $v->id]) }}" target="_blank" class="text-xs font-semibold text-zinc-500 dark:text-zinc-400 hover:text-orange-600">View</a>
                                <a href="{{ URL::temporarySignedRoute('documents.download', now()->addMinutes(5), ['document' => $v->id]) }}" class="text-xs font-semibold text-zinc-500 dark:text-zinc-400 hover:text-orange-600">Download</a>
                            </div>
                        </div>
                    @endforeach
                </div>
    ...
    @if($preview !== null)
        <div class="fixed inset-0 z-[60] flex items-center justify-center p-4" x-data x-on:keydown.escape.window="$wire.closePreview()">
            ...
            <div class="relative flex w-full max-w-4xl flex-col rounded-2xl bg-white dark:bg-zinc-900 shadow-2xl ring-1 ring-black/5 dark:bg-zinc-900 dark:ring-zinc-700">
                <div class="flex items-center justify-between border-b border-zinc-100 dark:border-zinc-800 px-5 py-3 dark:border-zinc-800">
                    <h3 class="truncate text-sm font-bold text-zinc-900 dark:text-white">{{ $preview['document']->title }}</h3>
                    <div class="flex items-center gap-3">
                        <a href="{{ URL::temporarySignedRoute('documents.download', now()->addMinutes(5), ['document' => $preview['document']->id]) }}" class="text-xs font-semibold text-zinc-500 dark:text-zinc-400 hover:text-orange-600">Download</a>
                        <button type="button" @click="$wire.closePreview()" class="text-zinc-400 hover:text-zinc-600 dark:text-zinc-300 dark:hover:text-zinc-200">
                            <svg class="size-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/></svg>
                        </button>
                    </div>
                </div>
                <div class="bg-zinc-100 dark:bg-zinc-800 p-2 dark:bg-zinc-950">
                    @if($preview['kind'] === 'pdf')
                        <iframe src="{{ $preview['url'] }}" class="h-[75vh] w-full rounded-lg bg-white dark:bg-zinc-900" title="Document preview"></iframe>
                    ...
                    @else
                        <div class="flex h-48 flex-col items-center justify-center gap-3 text-center">
                            <p class="text-sm text-zinc-500 dark:text-zinc-400">Preview isn't available for this file type.</p>
...
<dialog id="modal-upload" class="rounded-2xl shadow-2xl border-0 p-0 w-full max-w-lg backdrop:bg-black/40 backdrop:backdrop-blur-sm"
    ...
    <form method="POST" action="{{ route('documents.upload') }}" enctype="multipart/form-data"
        ...
        <div class="flex items-center justify-between">
            <div>
                <h2 class="text-lg font-bold text-zinc-900 dark:text-white">Upload Document</h2>
                <p class="text-sm text-zinc-500 dark:text-zinc-400 mt-0.5">PDF and images only · Max 10 MB</p>
            </div>
            <button type="button" onclick="document.getElementById('modal-upload').close()"
                class="rounded-lg p-2 text-zinc-400 hover:bg-zinc-100 dark:bg-zinc-800 hover:text-zinc-600 dark:text-zinc-300 transition-colors">
        ...
        <div>
            <label class="block text-sm font-medium text-zinc-700 dark:text-zinc-200 mb-1">Title <span class="text-red-500">*</span></label>
        ...
        <div>
            <label class="block text-sm font-medium text-zinc-700 dark:text-zinc-200 mb-1">Description</label>
        ...
        <div>
            <label class="block text-sm font-medium text-zinc-700 dark:text-zinc-200 mb-1">File <span class="text-red-500">*</span></label>
        ...
        <div class="grid grid-cols-2 gap-4">
            <div>
                <label class="block text-sm font-medium text-zinc-700 dark:text-zinc-200 mb-1">Category <span class="text-red-500">*</span></label>
            ...
            <div>
                <label class="block text-sm font-medium text-zinc-700 dark:text-zinc-200 mb-1">Visibility</label>
        ...
        <div x-show="vis === 'restricted'" x-transition style="display:none">
            <label class="block text-sm font-medium text-zinc-700 dark:text-zinc-200 mb-1">Assign to Employee <span class="text-zinc-400 font-normal text-xs">(optional)</span></label>
        ...
        <div class="grid grid-cols-2 gap-4">
            <div>
                <label class="block text-sm font-medium text-zinc-700 dark:text-zinc-200 mb-1">Expiry Date <span class="text-zinc-400 font-normal text-xs">(optional)</span></label>
            ...
            <div class="flex items-end pb-2">
                <label class="flex items-center gap-2 cursor-pointer select-none">
                    ...
                    <span class="text-sm text-zinc-700 dark:text-zinc-200">Requires acknowledgement</span>
                </label>
            </div>
        </div>
        ...
        <div class="flex justify-end gap-3 border-t border-zinc-100 dark:border-zinc-800 pt-4">
            <button type="button" onclick="document.getElementById('modal-upload').close()"
                class="rounded-lg px-4 py-2 text-sm font-medium text-zinc-600 dark:text-zinc-300 hover:bg-zinc-100 dark:bg-zinc-800 transition-colors">
                Cancel
            </button>
            <button type="submit"
...
<dialog id="modal-personal" class="rounded-2xl shadow-2xl border-0 p-0 w-full max-w-md backdrop:bg-black/40 backdrop:backdrop-blur-sm">
    <form method="POST" action="{{ route('documents.upload-personal') }}" enctype="multipart/form-data"
        ...
        <div class="flex items-center justify-between">
            <div>
                <h2 class="text-lg font-bold text-zinc-900 dark:text-white">Upload My Document</h2>
                <p class="text-sm text-zinc-500 dark:text-zinc-400 mt-0.5">Personal document — only you and HR can see it.</p>
            </div>
        ...
        <div>
            <label class="block text-sm font-medium text-zinc-700 dark:text-zinc-200 mb-1">Document Title <span class="text-red-500">*</span></label>
        ...
        <div>
            <label class="block text-sm font-medium text-zinc-700 dark:text-zinc-200 mb-1">Notes <span class="text-zinc-400 font-normal text-xs">(optional)</span></label>
