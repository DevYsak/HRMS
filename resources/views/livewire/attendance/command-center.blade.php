...
<div class="mb-4 grid grid-cols-2 gap-3 sm:grid-cols-3 xl:grid-cols-7">
    @foreach($tabs as $key => [$label, $icon, $color])
        <button wire:click="$set('tab', '{{ $key }}')"
            class="flex items-center gap-3 rounded-[18px] border p-4 text-left shadow-sm transition hover:-translate-y-0.5 hover:shadow-md {{ $tab === $key ? 'border-orange-400 bg-white dark:bg-zinc-900 ring-1 ring-orange-200' : 'border-orange-100/70 bg-white dark:bg-zinc-900' }}">
            ...
            <div>
                <div class="text-xl font-black tabular-nums text-zinc-900 dark:text-white">{{ $counts[$key] }}</div>
    ...
    <div class="flex items-center gap-3 rounded-[18px] border border-orange-100/70 bg-white dark:bg-zinc-900 p-4 shadow-sm">
        ...
        <div><div class="text-xl font-black tabular-nums text-zinc-900 dark:text-white">{{ $decided['approved'] }}</div><div class="text-[9px] font-bold uppercase tracking-wide text-zinc-400">Approved Requests</div></div>
    </div>
    <div class="flex items-center gap-3 rounded-[18px] border border-orange-100/70 bg-white dark:bg-zinc-900 p-4 shadow-sm">
        ...
        <div><div class="text-xl font-black tabular-nums text-zinc-900 dark:text-white">{{ $decided['rejected'] }}</div><div class="text-[9px] font-bold uppercase tracking-wide text-zinc-400">Rejected Requests</div></div>
    </div>
</div>
...
<div class="grid grid-cols-1 items-start gap-4 lg:grid-cols-12">
        ...
        @if(count($selected))
            <div class="flex flex-wrap items-center gap-2 border-b border-orange-100/70 bg-orange-50/60 px-5 py-2.5">
                ...
                <input type="text" wire:model="rejectComment" placeholder="Rejection comment (required for reject)…"
                    class="min-w-0 flex-1 rounded-lg border border-orange-200 bg-white dark:bg-zinc-900 px-2.5 py-1.5 text-xs focus:border-orange-400 focus:ring-0">
    ...
    <div class="rounded-[18px] border border-orange-100/70 bg-white dark:bg-zinc-900 p-5 shadow-sm lg:col-span-4">
        <div class="mb-3 flex items-center gap-2">
            ...
            <div class="text-sm font-black text-zinc-900 dark:text-white">Activity Feed</div>
        </div>
        @if(count($feed) > 0)
