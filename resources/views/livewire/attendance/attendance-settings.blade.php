<flux:main class="min-h-screen bg-zinc-50 dark:bg-zinc-950 p-6 space-y-6">
    ...
    <div class="grid grid-cols-1 gap-6 lg:grid-cols-3">
        ...
        <div class="rounded-2xl border border-zinc-100 bg-white p-6 shadow-sm dark:border-zinc-800 dark:bg-zinc-900">
            ...
            <div
                class="mb-4 rounded-xl border border-zinc-100 bg-zinc-50 p-3 text-xs text-zinc-500 dark:border-zinc-800 dark:bg-zinc-800/50">
        ...
        <div class="space-y-4 lg:col-span-2">
            <div
                class="rounded-2xl border border-zinc-100 bg-white p-6 shadow-sm dark:border-zinc-800 dark:bg-zinc-900">
                <div class="mb-5 flex items-center gap-2">
                    <div class="rounded-lg bg-sky-50 p-1.5 dark:bg-sky-900/20">
                    ...
                    @foreach($offices as $office)
                        <div class="rounded-xl border border-zinc-100 bg-zinc-50/50 p-4 dark:border-zinc-800 dark:bg-zinc-800/30"
                            ...
                            <div class="mb-4 flex items-start justify-between">
                                ...
                                <flux:button x-on:click="$wire.updateOffice(id, lat, lng, radius)" size="sm" variant="ghost"
                                    icon="arrow-path">
                                    Update
                                </flux:button>
                            </div>
    ...
    <div class="rounded-2xl border border-zinc-100 bg-white shadow-sm dark:border-zinc-800 dark:bg-zinc-900">
        ...
        <div class="flex items-center justify-between border-b border-zinc-50 px-6 py-4 dark:border-zinc-800">
            <div class="flex items-center gap-3">
                ...
                <div>
                    ...
                    <p class="text-[11px] text-zinc-400">Define named shifts â€” each employee is assigned one, which
                        controls their late-detection time.</p>
                </div>
            </div>
        ...
        <div
            class="mx-6 mt-4 rounded-xl border border-blue-100 bg-blue-50 p-4 dark:border-blue-900/40 dark:bg-blue-950/20">
            <div class="flex gap-3">
                ...
                <div class="text-xs leading-relaxed text-blue-700 dark:text-blue-300">
                    ...
                    If clock-in time is beyond <strong>Start Time + Grace Period</strong>, they are flagged <span
                        class="font-black text-amber-600">LATE</span>.<br>
                ...
                <thead>
                    <tr class="border-b border-zinc-50 bg-zinc-50/50 dark:border-zinc-800/50 dark:bg-zinc-950/30">
                        <th class="py-3 pl-6 pr-4 text-[10px] font-black uppercase tracking-[0.12em] text-zinc-400">
                            Shift Name</th>
                        <th class="px-4 py-3 text-[10px] font-black uppercase tracking-[0.12em] text-zinc-400">Start
                            Time</th>
                        <th class="px-4 py-3 text-[10px] font-black uppercase tracking-[0.12em] text-zinc-400">End Time
                        </th>
                        <th
                ...
                <tbody class="divide-y divide-zinc-50 dark:divide-zinc-800/30">
                    @forelse($shifts as $shift)
                        <tr class="transition-colors hover:bg-zinc-50/60 dark:hover:bg-zinc-800/20">
                            ...
                            <td class="px-4 py-4">
                                <div
                                    class="inline-flex items-center gap-1.5 rounded-lg bg-emerald-50 px-2.5 py-1 text-xs font-black text-emerald-700 dark:bg-emerald-900/20 dark:text-emerald-400">
                            ...
                            <td class="px-4 py-4 text-center">
                                <span
                                    class="inline-flex items-center gap-1 rounded-full bg-amber-50 px-2.5 py-1 text-[11px] font-black text-amber-700 dark:bg-amber-900/20 dark:text-amber-400">
                            ...
                            <td class="px-4 py-4 text-center">
                                <span
                                    class="text-sm font-black text-zinc-900 dark:text-white">{{ (float) $shift->standard_hours }}h</span>
                            </td>
                            <td class="px-4 py-4 text-center">
                                <span
                                    class="inline-flex size-7 items-center justify-center rounded-full bg-zinc-100 text-xs font-black text-zinc-600 dark:bg-zinc-800 dark:text-zinc-300">
                                    {{ $shift->employees_count }}
                                </span>
                            </td>
                            <td class="py-4 pr-6 text-right">
                                <div class="flex items-center justify-end gap-2">
                                    <flux:button wire:click="editShift({{ $shift->id }})" size="sm" variant="ghost"
                                        icon="pencil-square">
                                        Edit
                                    </flux:button>
                                    <flux:button wire:click="deleteShift({{ $shift->id }})"
                                        wire:confirm="Delete '{{ $shift->name }}'? This cannot be undone." size="sm"
                                        variant="ghost" class="text-rose-500 hover:text-rose-700">
                                        Delete
                                    </flux:button>
                                </div>
                            </td>
                            ...
                            <td colspan="7" class="py-14 text-center">
                                <div class="flex flex-col items-center gap-3">
                                    ...
                                    <div>
                                        ...
                                        <p class="mt-0.5 text-xs text-zinc-300 dark:text-zinc-600">Click "New Shift" to add
                                            IT Shift, UK Sales Shift, etc.</p>
                                    </div>
                                </div>
                            </td>
                        </tr>
    ...
    @if($showShiftModal)
        <div class="fixed inset-0 z-50 flex items-center justify-center p-4" x-data
            ...
            <div class="absolute inset-0 bg-black/40 backdrop-blur-sm" @click="$wire.set('showShiftModal', false)"></div>
            <div
                ...
                <button type="button" @click="$wire.set('showShiftModal', false)"
                ...
                <div class="space-y-6">
                    ...
                    <div class="space-y-4">
                        <flux:input wire:model="shiftName" label="Shift Name" placeholder="e.g. IT Shift, UK Sales Shift"
                            required />
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            <flux:field>
                                ...
                                <flux:description>Clock-in allowed within this buffer after Start Time without being marked
                                    Late.</flux:description>
                            </flux:field>
                            <flux:field>
                                <flux:label>Standard Hours</flux:label>
                                <flux:input wire:model="shiftStandardHours" type="number" step="0.5" min="1" max="24"
                                    suffix="hrs" required />
                                <flux:description>Expected net work hours per day (used for OT calculation).
                                </flux:description>
                            </flux:field>
                        </div>

                        <flux:input wire:model="shiftDescription" label="Description (optional)"
                            placeholder="e.g. UK sales team aligned to BST business hours" />

                        {{-- Live preview --}}
                        @if($shiftStart && $shiftEnd)
                            <div
                                class="rounded-xl border border-violet-100 bg-violet-50 p-3 dark:border-violet-900/40 dark:bg-violet-950/20">
                                <p class="text-xs font-bold text-orange-700 dark:text-violet-300">
