...
<div class="mb-4 flex flex-wrap items-center justify-between gap-3">
    <div>
        <h1 class="text-2xl font-black tracking-tight text-zinc-900">Attendance Reports</h1>
    ...
    <div class="flex flex-wrap items-center gap-2">
        <a href="{{ route('reports.attendance-report-csv', $this->exportParams) }}"
           class="inline-flex items-center gap-1.5 rounded-xl border border-orange-100 bg-white px-3 py-2 text-xs font-bold text-zinc-600 shadow-sm transition hover:bg-orange-50"><flux:icon.table-cells class="size-4 text-emerald-500" /> CSV / Excel</a>
        <a href="{{ route('reports.attendance-report-pdf', $this->exportParams) }}" target="_blank"
           class="inline-flex items-center gap-1.5 rounded-xl border border-orange-100 bg-white px-3 py-2 text-xs font-bold text-zinc-600 shadow-sm transition hover:bg-orange-50"><flux:icon.document-arrow-down class="size-4 text-rose-500" /> PDF</a>
        <button onclick="window.print()"
           class="inline-flex items-center gap-1.5 rounded-xl border border-orange-100 bg-white px-3 py-2 text-xs font-bold text-zinc-600 shadow-sm transition hover:bg-orange-50 print:hidden"><flux:icon.printer class="size-4 text-zinc-500" /> Print</button>
    </div>
</div>
...
<div class="mb-4 flex flex-wrap gap-2 print:hidden">
    @foreach($types as $key => $label)
        <button wire:click="$set('type', '{{ $key }}')"
            class="rounded-xl border px-3 py-1.5 text-xs font-bold transition {{ $type === $key ? 'border-orange-400 bg-orange-500 text-white shadow' : 'border-orange-100 bg-white text-zinc-500 hover:bg-orange-50' }}">{{ $label }}</button>
    @endforeach
</div>

{{-- ═══════════════ FILTERS ═══════════════ --}}
<div class="mb-4 rounded-[18px] border border-orange-100/70 bg-white p-4 shadow-sm print:hidden">
    <div class="grid grid-cols-2 gap-3 sm:grid-cols-3 xl:grid-cols-7">
        <div>
            ...
            <input type="date" wire:model.live="from" class="w-full rounded-lg border border-orange-100 bg-white px-2.5 py-1.5 text-xs font-semibold text-zinc-600 focus:border-orange-400 focus:ring-0">
        </div>
        <div>
...
{{-- ═══════════════ TREND CHART ═══════════════ --}}
@if($trendChart)
    <div class="mb-4 rounded-[18px] border border-orange-100/70 bg-white p-5 shadow-sm">
        <div class="mb-1 text-sm font-black text-zinc-900">Trend</div>
                ...
                <tbody class="divide-y divide-zinc-50">
                    @foreach($previewRows as $row)
                        <tr class="transition hover:bg-orange-50/40">
                            @foreach($row as $cell)<td class="whitespace-nowrap px-4 py-2 text-zinc-700">{{ $cell }}</td>@endforeach
                        </tr>
                    @endforeach
                </tbody>
            </table>
