<flux:main class="min-h-screen bg-zinc-50 dark:bg-zinc-950">
    ...
    {{-- ══════════════════════════════════════════
         KPI STATS BAR
    ══════════════════════════════════════════ --}}
    <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-0 border-b border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900">
        @php
            $kpis = [
        ...
        @foreach($kpis as $i => $kpi)
            <div class="flex items-center gap-3 px-5 py-5 {{ $i < 4 ? 'border-r border-zinc-100 dark:border-zinc-800' : '' }} {{ $i > 1 ? 'hidden sm:flex' : '' }} {{ $i > 2 ? 'hidden lg:flex' : '' }}">
    ...
    <div class="p-4 md:p-6 space-y-4">
        ...
        @if($pendingRegularisations->isNotEmpty())
            <div class="bg-white dark:bg-zinc-900 border border-amber-200 dark:border-amber-800/50 rounded-2xl overflow-hidden">
                <div class="flex items-center gap-3 px-5 py-3.5 border-b border-amber-100 dark:border-amber-900/30 bg-amber-50/50 dark:bg-amber-950/20">
                    <div class="size-7 rounded-lg bg-amber-100 dark:bg-amber-900/30 flex items-center justify-center shrink-0">
                        <flux:icon.clock class="size-4 text-amber-600 dark:text-amber-400" />
                    </div>
                    <div class="flex-1">
                        <span class="text-sm font-bold text-zinc-800 dark:text-zinc-200">Pending Regularisation Requests</span>
                    </div>
                    <span class="px-2 py-0.5 bg-amber-100 dark:bg-amber-900/40 text-amber-700 dark:text-amber-400 text-[10px] font-black rounded-full">
                    ...
                    <table class="w-full text-sm">
                        <thead>
                            <tr class="border-b border-zinc-100 dark:border-zinc-800">
                                <th class="py-2.5 pl-5 pr-4 text-left text-[10px] font-bold uppercase tracking-widest text-zinc-400">Employee</th>
                                <th class="py-2.5 pr-4 text-left text-[10px] font-bold uppercase tracking-widest text-zinc-400">Date</th>
                                <th class="py-2.5 pr-4 text-left text-[10px] font-bold uppercase tracking-widest text-zinc-400">Requested Time</th>
                                <th class="py-2.5 pr-4 text-left text-[10px] font-bold uppercase tracking-widest text-zinc-400">Reason</th>
                                <th class="py-2.5 pr-5 text-right text-[10px] font-bold uppercase tracking-widest text-zinc-400">Action</th>
                            </tr>
                        </thead>
                                ...
                                <tr class="hover:bg-amber-50/30 dark:hover:bg-amber-950/10 transition-colors">
                                    <td class="py-3 pl-5 pr-4">
                                        <div class="flex items-center gap-2">
                                            <div class="size-7 rounded-full bg-amber-100 dark:bg-amber-900/30 flex items-center justify-center text-[10px] font-black text-amber-700 dark:text-amber-400 shrink-0">
                                                {{ collect(explode(' ', $req->employee->user->name))->map(fn($n) => $n[0] ?? '')->take(2)->join('') }}
                                            </div>
                                            <span class="text-xs font-semibold text-zinc-800 dark:text-zinc-200">{{ $req->employee->user->name }}</span>
                                        </div>
                                    </td>
                                    <td class="py-3 pr-4 text-xs text-zinc-600 dark:text-zinc-400">{{ \Carbon\Carbon::parse($req->work_date)->format('d M Y') }}</td>
                                    <td class="py-3 pr-4">
                                        <span class="font-mono text-xs text-amber-700 dark:text-amber-400 bg-amber-50 dark:bg-amber-950/30 px-2 py-0.5 rounded">
        ...
        <div class="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-2xl px-5 py-3.5">
            <div class="flex flex-wrap items-center gap-3">
                ...
                <div class="ml-auto text-xs text-zinc-400 font-medium">
                    Showing {{ $attendances->firstItem() }}–{{ $attendances->lastItem() }} of {{ $attendances->total() }}
                </div>
            </div>
        </div>
        ...
        <div class="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-2xl overflow-hidden">
                ...
                <table class="w-full text-sm">
                    <thead>
                        <tr class="border-b border-zinc-100 dark:border-zinc-800 bg-zinc-50/60 dark:bg-zinc-950/40">
                            <th class="py-3 pl-5 pr-4 text-left text-[10px] font-bold uppercase tracking-widest text-zinc-400 min-w-[180px]">Employee</th>
                            ...
                            <th class="py-3 pr-4 text-left text-[10px] font-bold uppercase tracking-widest text-zinc-400">Check Out</th>
                            <th class="py-3 pr-4 text-left text-[10px] font-bold uppercase tracking-widest text-zinc-400">Work Hrs</th>
                            ...
                            <th class="py-3 pr-5 text-left text-[10px] font-bold uppercase tracking-widest text-zinc-400">Att. %</th>
                        </tr>
                    </thead>
                            ...
                            <tr class="hover:bg-zinc-50/60 dark:hover:bg-zinc-800/20 transition-colors group">
                                ...
                                <td class="py-3.5 pl-5 pr-4">
                                    <div class="flex items-center gap-2.5">
                                        <div class="size-9 rounded-xl {{ $avatarBg }} flex items-center justify-center text-[11px] font-black shrink-0">
                                            {{ $initials }}
                                        </div>
                                        <div class="min-w-0">
                                            <div class="font-semibold text-sm text-zinc-900 dark:text-white truncate">{{ $log->employee->user->name }}</div>
                                            <div class="text-[10px] text-zinc-400">{{ $log->employee->employee_id ?? '—' }}</div>
                                        </div>
                                    </div>
                                </td>
                                ...
                                <td class="py-3.5 pr-4">
                                    ...
                                    @if($log->is_late)
                                        <div class="text-[10px] text-amber-600 font-bold">Late {{ $log->late_minutes ?? '' }}m</div>
                                    @elseif($log->check_in)
                                ...
                                <td class="py-3.5 pr-4">
                                    @if($log->check_out)
                                        <div class="font-mono text-sm font-bold text-zinc-900 dark:text-white tabular-nums">{{ $log->check_out->format('H:i') }}</div>
                                    @elseif($log->check_in)
                                        <span class="inline-flex items-center gap-1 text-[10px] font-black text-emerald-600">
                                            <span class="size-1.5 rounded-full bg-emerald-500 animate-pulse inline-block"></span> LIVE
                                        </span>
                                    @else
                                ...
                                <td class="py-3.5 pr-4">
                                    ...
                                    <span class="text-sm font-black tabular-nums {{ $hrs >= 8 ? 'text-emerald-600' : ($hrs > 0 ? 'text-amber-600' : 'text-zinc-300 dark:text-zinc-600') }}">
                                ...
                                <td class="py-3.5 pr-5">
                                    <div class="flex items-center gap-2">
                                        <div class="flex-1 h-1.5 bg-zinc-100 dark:bg-zinc-800 rounded-full overflow-hidden min-w-[40px]">
                                        ...
                                        <span class="text-xs font-bold text-zinc-700 dark:text-zinc-300 tabular-nums w-8 shrink-0">{{ $empPct }}%</span>
                                    </div>
                                </td>
            ...
            @if($attendances->hasPages())
                <div class="px-5 py-4 border-t border-zinc-100 dark:border-zinc-800 bg-zinc-50/50 dark:bg-zinc-950/30 flex items-center justify-between">
        ...
        <div class="space-y-5">
            ...
            <div class="flex gap-3 rounded-xl border border-amber-100 bg-amber-50 p-3 dark:border-amber-800/30 dark:bg-amber-900/10">
                ...
                <p class="text-xs text-amber-700 dark:text-amber-300 leading-relaxed">
                    This creates a <strong>Regularisation Request</strong> sent to the employee's <strong>Line Manager</strong> for approval.
                </p>
            </div>
            <div class="space-y-4">
