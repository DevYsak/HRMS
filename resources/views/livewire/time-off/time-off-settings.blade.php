                    ...
                    >
                        ...
                        <div class="flex-1 min-w-0">
                            <div class="flex items-center gap-2 flex-wrap mb-0.5">
                                ...
                                @if($type->code)
                                    <span class="px-1.5 py-0.5 text-[10px] font-mono font-bold bg-zinc-100 dark:bg-zinc-800 text-zinc-500 rounded">{{ $type->code }}</span>
                                @endif
                                ...
                                @if($type->is_monthly_accrual)
                                    <span class="px-1.5 py-0.5 text-[10px] font-bold bg-indigo-50 text-indigo-600 dark:bg-indigo-900/20 dark:text-indigo-400 rounded">Accrual</span>
                                @endif
                                @if($type->gender_restriction !== 'none')
                                    <span class="px-1.5 py-0.5 text-[10px] font-bold bg-pink-50 text-pink-600 dark:bg-pink-900/20 dark:text-pink-400 rounded">{{ ucfirst($type->gender_restriction) }} only</span>
                                @endif
                            </div>
                            <div class="text-xs text-zinc-500 dark:text-zinc-400 flex flex-wrap gap-x-3 gap-y-0.5">
                                <span>CF: {{ $type->allow_carry_forward ? ($type->carry_forward_limit > 0 ? 'Yes ('.$type->carry_forward_limit.'d)' : 'Yes') : 'No' }}</span>
                                ...
                                @if($type->attachment_required) <span class="text-amber-600 dark:text-amber-400">Attachment req.</span> @endif
            ...
            <form wire:submit="save" class="space-y-5">
                ...
                <div class="rounded-xl border border-zinc-200 dark:border-zinc-700 p-4 space-y-4">
                    ...
                    <div class="grid grid-cols-2 gap-3">
                        ...
                        <flux:input wire:model="code" label="Code" placeholder="e.g. EL" description="Short unique code" />
                    </div>
                    ...
                    <div class="flex items-center gap-2">
                        ...
                        <label for="is_paid_chk" class="text-sm font-semibold text-zinc-700 dark:text-zinc-300 cursor-pointer">Default Paid Leave</label>
                    </div>
                </div>
                ...
                <div class="rounded-xl border border-zinc-200 dark:border-zinc-700 p-4 space-y-3">
                    ...
                    <div class="grid grid-cols-2 gap-3">
                        <flux:input wire:model="carry_forward_limit" type="number" min="0" label="CF Limit (days, 0 = unlimited)" />
                        <flux:input wire:model="max_consecutive_days" type="number" min="1" label="Max Consecutive Days" placeholder="blank = no limit" />
                    </div>

                    {{-- Encashment config (visible only when encashment is enabled) --}}
                    @if($allow_encashment)
                        <div class="rounded-xl border border-blue-100 dark:border-blue-900/40 bg-blue-50/50 dark:bg-blue-900/10 p-4 space-y-3">
                            <p class="text-xs font-bold text-blue-500 uppercase tracking-wide">Encashment Rules</p>
                            <div class="grid grid-cols-2 gap-3">
                                <flux:input wire:model="max_encashable_days" type="number" min="1" max="365" label="Max Encashable Days / Year" placeholder="blank = no cap" />
                                <flux:input wire:model="encashment_rate_multiplier" type="number" step="0.05" min="0.1" max="10" label="Rate Multiplier (e.g. 1.5×)" />
                            </div>
                            <label class="flex items-center gap-2 cursor-pointer text-sm">
                                <flux:checkbox wire:model="allow_current_year_encashment" />
                                <span class="text-zinc-700 dark:text-zinc-300">Allow current-year leave encashment (in addition to carry-forward)</span>
                            </label>
                        </div>
                    @endif
                </div>
                ...
                <div class="rounded-xl border border-zinc-200 dark:border-zinc-700 p-4 space-y-3">
                    ...
                    @if($is_monthly_accrual)
                        <flux:input wire:model="accrual_days_per_month" type="number" step="0.25" min="0" max="31" label="Days to Credit Per Month" placeholder="e.g. 1.75" />
                    @endif
                </div>
                ...
                <div class="rounded-xl border border-zinc-200 dark:border-zinc-700 p-4 space-y-3">
                    ...
                    <label class="flex items-center gap-2 cursor-pointer text-sm">
                        ...
                        <div>
                            ...
                            <p class="text-xs text-zinc-400 mt-0.5">HR cannot manually adjust this leave type. Used for Unauthorized Leave, LWP, and WFH.</p>
                        </div>
                    </label>
                </div>

                <div class="flex justify-end gap-3 pt-2">
                    <button type="button" @click="$wire.set('showModal', false)" class="px-4 py-2 text-sm font-semibold text-zinc-600 dark:text-zinc-300 border border-zinc-200 dark:border-zinc-600 rounded-xl hover:bg-zinc-50 dark:hover:bg-zinc-700 transition-colors">Cancel</button>
