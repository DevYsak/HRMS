<flux:main class="min-h-screen bg-gradient-to-br from-slate-50 via-white to-indigo-50/30 dark:from-zinc-950 dark:via-zinc-950 dark:to-zinc-900 p-0"
    x-data="{
        scale: 1,
        pan: { x: 0, y: 0 },
        dragging: false,
    ...
    <div class="relative w-full overflow-hidden"
        ...
        <div class="flex min-w-max flex-col items-center px-24 py-16 transition-transform duration-150 origin-top select-none"
            ...
            @if($topLevel->isEmpty())
                <div class="flex flex-col items-center justify-center rounded-2xl border border-slate-200 bg-white dark:border-zinc-800 dark:bg-zinc-900 p-16 text-center shadow-sm">
                    <div class="mb-4 flex size-16 items-center justify-center rounded-2xl bg-slate-100 dark:bg-zinc-800">
                    ...
                    <h3 class="text-lg font-bold text-slate-700 dark:text-zinc-100">No hierarchy found</h3>
                    <p class="mt-1 text-sm text-slate-400 dark:text-zinc-500">No reporting lines found for your account.</p>
                </div>
            @else
                    ...
                    @foreach($topLevel as $manager)
                        ...
                        <div class="flex flex-col items-center">
                            ...
                            <div class="group relative w-72">
                                <div class="relative overflow-hidden rounded-2xl border-2 border-indigo-200 bg-white dark:border-indigo-500/30 dark:bg-zinc-900 p-5 shadow-xl shadow-indigo-100/60 dark:shadow-black/40 ring-4 ring-indigo-50 dark:ring-indigo-500/10 transition-all duration-300 group-hover:-translate-y-1 group-hover:shadow-2xl group-hover:shadow-indigo-200/70">
                                    {{-- Top gradient bar --}}
                                    ...
                                    <div class="flex items-center gap-4 pt-1">
                                        ...
                                        <div class="relative shrink-0">
                                            ...
                                            <img src="{{ $manager->user->avatarUrl() }}"
                                                 class="relative size-14 rounded-full border-2 border-white dark:border-zinc-900 object-cover shadow-md" />
                                            <div class="absolute -bottom-0.5 -right-0.5 size-3.5 rounded-full border-2 border-white dark:border-zinc-900 bg-emerald-400"></div>
                                        </div>

                                        <div class="min-w-0 flex-1">
                                            <h3 class="truncate text-base font-bold text-slate-900 dark:text-white">{{ $manager->user->name }}</h3>
                                            <p class="truncate text-[11px] font-semibold uppercase tracking-wide text-indigo-600">
                                                {{ $manager->jobTitle?->name ?? 'Director' }}
                                            </p>
                                            @if($manager->department)
                                                <span class="mt-1 inline-block rounded-full bg-indigo-50 px-2 py-0.5 text-[10px] font-semibold text-indigo-600 ring-1 ring-indigo-100 dark:bg-indigo-500/15 dark:text-indigo-300 dark:ring-indigo-500/20">
                                    ...
                                    @if($reportCount > 0)
                                        <div class="mt-3 flex items-center gap-1.5 border-t border-slate-100 dark:border-zinc-800 pt-3">
                                            ...
                                            <span class="text-[11px] font-medium text-slate-500 dark:text-zinc-400">{{ $reportCount }} direct {{ Str::plural('report', $reportCount) }}</span>
                                        </div>
                                    @endif
                                </div>
                            </div>
